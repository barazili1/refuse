...
export function AppleGame({ onBack, accessKeyData, language, onLanguageChange, platform }: AppleGameProps) {
  ...
  return (
    <div className={`flex flex-col h-full relative pt-0 select-none bg-[#050505] overflow-hidden ${isRtl ? 'font-ar' : 'font-en'}`}>
        ...
        <div className={`flex-1 overflow-y-auto custom-scrollbar pt-20 pb-28 px-6 relative z-10 flex flex-col ${isRtl ? 'text-right' : 'text-left'}`}>
            <motion.div layout initial={{ scale: 0.98, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="relative mb-8 group z-10 shrink-0 transform-gpu">
                <div className={`bg-black/40 p-0.5 rounded-[2.5rem] border transition-all duration-700 overflow-hidden min-h-[420px] flex flex-col justify-end ${isAnalyzing ? 'border-red-500/50' : 'border-white/10'}`}>
                ...
                <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="absolute -bottom-5 left-1/2 -translate-x-1/2 flex items-center gap-5 bg-black border border-red-500/30 px-6 py-2 rounded-2xl z-30 flex-row">
                    <div className="flex items-center gap-2.5 flex-row">
                        <div className={`w-2 h-2 rounded-full ${isAnalyzing ? 'bg-red-500' : 'bg-red-500/40'}`} />
            ...
            <div className="space-y-4 relative z-10 shrink-0 pb-10">
                <div className="grid grid-cols-2 gap-4 mb-2">
                    <div className="relative space-y-1.5">
                        ...
                        <button disabled={isAnalyzing} onClick={() => { playSound('click'); setIsRiskMenuOpen(!isRiskMenuOpen); setIsHeightMenuOpen(false); }} className={`w-full h-12 bg-black border rounded-2xl px-4 flex items-center justify-between group transition-all disabled:opacity-30 ${isRiskMenuOpen ? 'border-red-500/60' : 'border-white/10 hover:border-red-500/30'}`}>
                            <div className="flex items-center gap-2.5">
                                {difficulty === 'Easy' ? <Shield className="w-4 h-4 text-red-500" /> : <Zap className="w-4 h-4 text-red-500" />}
                                <span className="text-[10px] font-black text-white uppercase tracking-widest">{difficulty}</span>
                            </div>
                            <ChevronDown className={`w-4 h-4 text-zinc-600 transition-transform duration-500 ${isRiskMenuOpen ? 'rotate-180 text-red-500' : ''}`} />
                        </button>
                        <AnimatePresence>
                            {isRiskMenuOpen && (
                                <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} className="absolute bottom-[110%] left-0 right-0 z-[120] bg-black border border-red-500/30 rounded-2xl overflow-hidden">
                                    {['Easy', 'Pro'].map((mode) => (
                                        <button key={mode} onClick={() => handleDifficultySelect(mode as 'Easy' | 'Pro')} className={`w-full h-12 px-5 flex items-center justify-between text-[9px] font-black uppercase tracking-widest transition-all ${difficulty === mode ? 'bg-red-500 text-black' : 'text-zinc-500 hover:bg-red-500/10 hover:text-white'}`}>
                    ...
                    <div className="relative space-y-1.5">
                        ...
                        <button disabled={isAnalyzing} onClick={() => { playSound('click'); setIsHeightMenuOpen(!isHeightMenuOpen); setIsRiskMenuOpen(false); }} className={`w-full h-12 bg-black border rounded-2xl px-4 flex items-center justify-between group transition-all disabled:opacity-30 ${isHeightMenuOpen ? 'border-red-500/60' : 'border-white/10 hover:border-red-500/30'}`}>
                            <span className="text-[11px] font-black text-white font-mono italic">{rowCount}</span>
                            <ChevronDown className={`w-4 h-4 text-zinc-600 transition-transform duration-500 ${isHeightMenuOpen ? 'rotate-180 text-red-500' : ''}`} />
                        </button>
                        <AnimatePresence>
                            {isHeightMenuOpen && (
                                <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} className="absolute bottom-[110%] left-0 right-0 z-[120] bg-black border border-red-500/30 rounded-2xl overflow-hidden grid grid-cols-2 p-2 gap-2">
                                    {[5, 6, 7, 8, 9, 10].map((val) => (
                                        <button key={val} onClick={() => handleHeightSelect(val)} className={`h-11 rounded-xl flex items-center justify-center text-[11px] font-black font-mono transition-all ${rowCount === val ? 'bg-red-500 text-black' : 'text-zinc-500 hover:bg-red-500/10 hover:text-white bg-white/5'}`}>{val}</button>
                                    ))}
                                </motion.div>
                            )}
                        </AnimatePresence>
                ...
                <div className="flex flex-col gap-4">
                    <button onClick={handlePredict} disabled={isAnalyzing || isUpdating} className={`group relative w-full h-16 rounded-[2rem] overflow-hidden transition-all active:scale-[0.96] ${isAnalyzing ? 'bg-black cursor-wait border border-red-500/30' : 'bg-red-500 hover:bg-red-400'}`}>
                        {isAnalyzing && (
                            <motion.div 
                                initial={{ width: 0 }} 
                                animate={{ width: `${predictionProgress}%` }} 
                                className="absolute inset-y-0 bg-red-500/30 pointer-events-none transform-gpu" 
                            />
                        )}
                        <div className="relative z-10 flex flex-col items-center justify-center h-full">
                            {isAnalyzing ? (
                                <div className="flex items-center gap-4">
                                    ...
                                    <span className="text-[10px] font-black tracking-[0.6em] text-red-500 uppercase italic font-en">DECRYPTING_...</span>
                                </div>
                            ) : (
